...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for theme700
 */

// Auto-rebuild the theme registry during theme development.
if (theme_get_setting('theme700_clear_registry')) {
  // Rebuild .info data.
 ...
 */
function theme700_html_head_alter(&$head_elements) {
 ...
 */
function theme700_preprocess_search_block_form(&$vars) {
 ...
 */
function theme700_preprocess(&$vars, $hook) {
  $vars['theme700_path'] = base_path() . path_to_theme();
}

/**
 * Implements template_preprocess_html().
 */
function theme700_preprocess_html(&$vars) {
  
  $vars['doctype'] = _theme700_doctype();
  $vars['rdf'] = _theme700_rdf($vars);
  ...
  if (!$vars['is_front']) {
    ...
    if (isset($page_name)) {
      $vars['classes_array'][] = theme700_id_safe('page-' . $page_name);
    }

    $vars['classes_array'][] = theme700_id_safe('section-' . $section);
 ...
 */
function theme700_preprocess_page(&$vars) {
 ...
 */
function theme700_preprocess_maintenance_page(&$vars) {
  ...
  drupal_add_css(drupal_get_path('theme', 'theme700') . '/css/maintenance-page.css');
}
 ...
 */
function theme700_preprocess_node(&$vars) {
  // Striping class
 ...
 */
function theme700_preprocess_block(&$vars, $hook) {
 ...
 */
function theme700_menu_tree($vars) {
 ...
 */
function theme700_field__taxonomy_term_reference($vars) {
  $output = '';
 ...
 */
function theme700_breadcrumb($vars) {
  
  $breadcrumb = isset($vars['breadcrumb']) ? $vars['breadcrumb'] : array();

// Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('theme700_breadcrumb_display');
  if ($show_breadcrumb == 'yes') {
  
	  if (theme_get_setting('theme700_breadcrumb_hideonlyfront')) {
		$condition = count($breadcrumb) > 1;
	  } else {
		$condition = !empty($breadcrumb);
	  }
	  
	  if(theme_get_setting('theme700_breadcrumb_showtitle')) {
	  ...
	  $separator = theme_get_setting('theme700_breadcrumb_separator');
 ...
 * @return bool
 */
function theme700_tabs_float() {
  $float = (bool) theme_get_setting('theme700_tabs_float');
  $float_node = (bool) theme_get_setting('theme700_tabs_node');
...
function theme700_id_safe($string) {
  // Strip accents
 ...
 * @ingroup themeable
 */
function theme700_menu_link(array $vars) {
  ...
  $element['#attributes']['class'][] = theme700_id_safe($element['#title']);
 ...
 */
function theme700_preprocess_menu_local_task(&$vars) {
 ...
 */
function theme700_menu_local_tasks(&$vars) {
  $output = '';
